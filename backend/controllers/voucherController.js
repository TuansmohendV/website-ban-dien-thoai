...
import { notifyNewVoucherEmail, notifyNewVoucherInApp } from '../utils/marketingNotifications.js';
...
const mapVoucherForPublic = (voucher) => {
  const remainingUses =
    voucher.usageLimit > 0
      ? Math.max(voucher.usageLimit - voucher.usedCount, 0)
      : null;
...
export const getPublicVouchers = asyncHandler(async (req, res) => {
  ...
  res.json({
    data: vouchers.map(mapVoucherForPublic),
  });
});
...
export const createAdminVoucher = asyncHandler(async (req, res) => {
  ...
  if (voucher.isActive) {
    notifyNewVoucherEmail(voucher).catch((error) => {
      console.error('Failed to send new voucher email notifications:', error);
    });
    notifyNewVoucherInApp(voucher).catch((error) => {
      console.error('Failed to create new voucher in-app notifications:', error);
    });
  }
