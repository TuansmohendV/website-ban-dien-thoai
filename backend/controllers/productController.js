...
import { notifyNewProductEmail, notifyNewProductInApp } from '../utils/marketingNotifications.js';
...
export const createAdminProduct = asyncHandler(async (req, res, next) => {
  ...
  notifyNewProductEmail(product).catch((error) => {
    console.error('Failed to send new product email notifications:', error);
  });
  notifyNewProductInApp(product).catch((error) => {
    console.error('Failed to create new product in-app notifications:', error);
  });
