...
export const getReviewsByProductId = asyncHandler(async (req, res) => {
  ...
  const [total, reviews] = await Promise.all([
    Review.countDocuments({ product: req.params.productId, isDeleted: { $ne: true } }),
    Review.find({ product: req.params.productId, isDeleted: { $ne: true } })
...
export const getMyReviews = asyncHandler(async (req, res) => {
  const reviews = await Review.find({ user: req.user._id, isDeleted: { $ne: true } })
...
export const deleteAdminReview = asyncHandler(async (req, res) => {
  ...
  review.deletedAt = new Date();
  review.moderationStatus = 'rejected';
  review.moderationNote = review.moderationNote || 'Đã ẩn bởi quản trị viên.';
  await review.save();
  await syncProductReviewStats(review.product);
