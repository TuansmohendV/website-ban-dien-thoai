...
const ProfilePage = () => {
    ...
    const sidebarItems = [
        ...
        { id: 'addresses', label: 'Sổ địa chỉ giao hàng', icon: <MapPin size={20} /> },
    ...
    const loadTabData = async (tabId) => {
        if (!['vouchers', 'addresses', 'history', 'comments', 'ratings', 'notifications'].includes(tabId)) {
            return;
        }
    ...
    useEffect(() => {
        const handleOpenProfileTab = (event) => {
            if (event.detail === 'notifications') {
                setActiveTab('notifications');
            }
        };

        window.addEventListener('phonesin:open-profile-tab', handleOpenProfileTab);
        return () => window.removeEventListener('phonesin:open-profile-tab', handleOpenProfileTab);
    }, []);
            ...
            <div className="max-w-[1500px] mx-auto px-4 sm:px-6">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <main className="flex-1">
                        ...
                        {activeTab === 'notifications' && (
                            <div className="space-y-6 animate-in fade-in duration-300">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                    <div>
                                        <h2 className="text-2xl sm:text-[28px] font-black text-gray-900 tracking-tight">Thông báo</h2>
                                        <p className="text-sm font-bold text-gray-400 mt-1">Theo dõi sản phẩm mới, voucher, đơn hàng và hỗ trợ.</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={handleMarkAllNotificationsRead}
                                        className="px-5 py-3 bg-[#008d71] text-white rounded-xl text-sm font-black hover:bg-[#006e58] transition-all disabled:opacity-50"
                                ...
                                {tabLoading ? (
                                    ...
                                    <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
                                        <Bell size={42} className="mx-auto text-gray-300 mb-3" />
                                        <p className="font-black text-gray-900">Bạn chưa có thông báo nào.</p>
                                    </div>
                                ) : (
                                    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                                        {notifications.map((notification) => (
                                            <div
                                                ...
                                                className={`p-5 border-b border-gray-100 last:border-b-0 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 ${
                                                    notification.isRead ? 'bg-white' : 'bg-emerald-50/60'
                                                }`}
                                            >
                                                <div className="flex items-start gap-3 min-w-0">
                                                    <span className={`mt-2 w-2.5 h-2.5 rounded-full shrink-0 ${notification.isRead ? 'bg-gray-200' : 'bg-[#008d71]'}`} />
                                                    <div className="min-w-0">
                                                        <h3 className="text-[15px] font-black text-gray-900">{notification.title}</h3>
                                                        <p className="text-sm font-semibold text-gray-500 leading-relaxed mt-1">{notification.message}</p>
                                                ...
                                                {!notification.isRead && (
                                                    <button
                                                        ...
                                                        className="shrink-0 px-4 py-2 bg-white border border-[#008d71]/20 text-[#008d71] rounded-lg text-xs font-black hover:border-[#008d71] transition-all"
                                                    >
                                                        Đã đọc
                                                    </button>
                                                )}
                        ...
                        {activeTab === 'vouchers' && (
                            <div className="space-y-6 animate-in fade-in duration-300">
                                <h2 className="text-2xl sm:text-[28px] font-black text-gray-900 tracking-tight">Trung tâm voucher</h2>
                                <p className="text-[15px] font-medium text-gray-600 leading-relaxed">
                                    Xem các chương trình giảm giá theo khung giờ và mã ưu đãi. Bạn cũng có thể nhập mã tại bước thanh toán đơn hàng.
                                </p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <Link
                                        to="/flash-voucher"
                                        className="group bg-gradient-to-br from-[#008d71] to-[#006b57] rounded-2xl p-6 text-white shadow-lg border border-white/10 hover:shadow-xl transition-all"
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <p className="text-[11px] font-black uppercase tracking-widest text-white/80 mb-2">Ưu đãi giới hạn</p>
                                                <p className="text-xl font-black leading-snug">Săn voucher Flash Sale</p>
                                                <p className="text-sm font-semibold text-white/85 mt-2">Mở trang khuyến mãi theo khung giờ.</p>
                                            </div>
                                            <Ticket size={36} className="opacity-90 shrink-0 group-hover:scale-110 transition-transform" />
                                        </div>
                                        <span className="inline-flex mt-5 text-sm font-black underline underline-offset-4">Đi tới trang voucher →</span>
                                    </Link>
                                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-center">
                                        <p className="text-[13px] font-black text-gray-500 uppercase tracking-wide mb-2">Thanh toán</p>
                                        <p className="text-[15px] font-bold text-gray-800 leading-relaxed">
                                            Ở bước đặt hàng, chọn &quot;Áp dụng mã giảm giá&quot; và nhập mã admin gửi hoặc mã bạn đã lưu.
                                        </p>
                                        <Link
                                            to="/cart"
                                            className="mt-4 inline-flex items-center justify-center h-11 px-5 rounded-xl bg-gray-900 text-white text-sm font-black hover:bg-black transition-colors"
                                        >
                                            Đi đến giỏ hàng
                                        </Link>
                                    </div>
