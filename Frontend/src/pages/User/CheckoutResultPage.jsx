...
const extractAppOrderId = (params) => {
    if (params.extraData) {
        try {
            const decoded = JSON.parse(window.atob(params.extraData));
            if (decoded.orderId) {
                return decoded.orderId;
            }
        } catch {
            // Fall back to provider order id parsing.
        }
    }

    if (params.orderId) {
        return String(params.orderId).split('_')[0];
    }

    if (params.vnp_TxnRef) {
        return String(params.vnp_TxnRef).split('_')[0];
    }
    ...
    useEffect(() => {
        const verifyPayment = async () => {
            try {
                ...
                const finalOrderId = extractAppOrderId(allParams);
                const paymentMethod = params.get('method') || (allParams.vnp_TxnRef ? 'vnpay' : allParams.partnerCode ? 'MOMO' : 'unknown');
