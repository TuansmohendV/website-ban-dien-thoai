...
const VoucherCard = ({ voucher, nowSec }) => {
  ...
  const usedPct = Math.round((voucher.used / voucher.qty) * 100);
  const remaining_qty = voucher.qty - voucher.used;
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            ...
            <div className="flex items-end gap-2">
              <span className="text-6xl font-black leading-none italic tracking-tighter">{voucher.discount}%</span>
            ...
            <p className="text-xs text-white/60 font-bold mt-2">
              Tối đa {formatPrice(voucher.maxDiscount)} • Từ {formatPrice(voucher.minOrder)}
            </p>
          </div>
      ...
      <div className="px-7 py-6 space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-[11px] font-black text-gray-400 uppercase tracking-widest">
            ...
            <span className="text-red-500">{remaining_qty > 0 ? `Còn ${remaining_qty} mã` : 'Hết mã!'}</span>
          </div>
