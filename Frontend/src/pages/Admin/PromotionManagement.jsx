...
const PromotionManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tắt mã khuyến mãi này khỏi giao diện áp dụng? Dữ liệu vẫn được giữ trong hệ thống.')) {
      try {
        await api.delete(`/api/voucher/admin/${id}`);
        setPromos(promos.filter(p => p.id !== id));
      } catch (error) {
        alert(getApiErrorMessage(error, 'Không thể tắt mã khuyến mãi.'));
      }
    }
  };
                ...
                <tr key={promo.id}>
                  ...
                  <td>
                    <div className="actions-cell">
                      <button className="action-btn" title="Copy mã" onClick={() => copyToClipboard(promo.code)} disabled={!promo.isActive} style={{ opacity: promo.isActive ? 1 : 0.5, cursor: promo.isActive ? 'pointer' : 'not-allowed' }}><Copy size={16} /></button>
