...
const FeedbackManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Ẩn bình luận này khỏi giao diện? Dữ liệu vẫn được giữ trong hệ thống.')) {
      try {
        await api.delete(`/api/admin/reviews/${id}`);
        await loadComments();
      } catch (error) {
        alert(getApiErrorMessage(error, 'Không thể ẩn bình luận.'));
      }
    }
  };

  const handleDeleteNotification = async (id) => {
    if (window.confirm('Ẩn thông báo này khỏi giao diện? Dữ liệu vẫn được giữ trong hệ thống.')) {
      try {
        await api.delete(`/api/admin/broadcasts/${id}`);
        await loadNotifications();
      } catch (error) {
        alert(getApiErrorMessage(error, 'Không thể ẩn thông báo.'));
      }
    }
  };
