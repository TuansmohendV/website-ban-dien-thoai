...
import { Bell, CheckCheck, Search, MapPin, User, ShoppingCart } from 'lucide-react';
...
const Navbar = () => {
    ...
    const currentUserId = user?.id || user?._id || user?.backendId || '';
    ...
    const getNotificationTarget = (notification) => {
        if (notification.relatedType === 'voucher') return '/flash-voucher';
        if (notification.relatedType === 'order') return '/orders';
        if (notification.relatedType === 'supportTicket') return '/support';
        return '/profile';
    };
                        ...
                        <div className="flex items-center justify-end gap-2 sm:gap-4 lg:gap-7 shrink-0 h-9 sm:h-11 lg:h-full min-w-0">
                            ...
                            <div className="relative h-full" ref={notificationRef}>
                                <button
                                    ...
                                    className="relative h-full flex items-center justify-center text-[#008d71] hover:text-[#006e58] transition-colors"
                                    title="Thông báo"
                                >
                                    <Bell size={20} strokeWidth={1.7} />
                                    {unreadCount > 0 && (
                                        <span className="absolute -top-1.5 -right-2 min-w-[18px] h-[18px] px-1 bg-[#ff424e] text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                                ...
                                {showNotifications && (
                                    <div className="absolute right-0 top-[calc(100%+12px)] w-[340px] max-w-[calc(100vw-24px)] bg-white border border-gray-100 rounded-2xl shadow-2xl z-[120] overflow-hidden">
                                        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3">
                                            <div>
                                                <h3 className="text-[15px] font-black text-gray-900">Thông báo</h3>
                                                <p className="text-[11px] font-bold text-gray-400">{unreadCount} thông báo chưa đọc</p>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={handleMarkAllNotificationsRead}
                                                className="inline-flex items-center gap-1 text-[11px] font-black text-[#008d71] hover:underline"
                                            >
                                                <CheckCheck size={14} />
                                                Đọc hết
                                            </button>
                                        ...
                                        <div className="max-h-[360px] overflow-y-auto">
                                            {isLoadingNotifications ? (
                                                <div className="p-5 text-center text-[13px] font-bold text-gray-400">Đang tải thông báo...</div>
                                            ) : notifications.length === 0 ? (
                                                <div className="p-5 text-center text-[13px] font-bold text-gray-400">Chưa có thông báo mới.</div>
                                            ) : (
                                                notifications.map((notification) => (
                                                    ...
                                                    >
                                                        <div className="flex items-start gap-3">
                                                            <span className={`mt-1 w-2 h-2 rounded-full shrink-0 ${notification.isRead ? 'bg-gray-200' : 'bg-[#008d71]'}`} />
                                                            <div className="min-w-0">
                                                                <p className="text-[13px] font-black text-gray-900 line-clamp-1">{notification.title}</p>
                                                                <p className="text-[12px] font-semibold text-gray-500 leading-relaxed line-clamp-2">{notification.message}</p>
                                        ...
                                        <div className="px-4 py-3 bg-gray-50 text-center">
                                            <Link
                                                ...
                                                onClick={() => {
                                                    setShowNotifications(false);
                                                    window.setTimeout(() => {
                                                        window.dispatchEvent(new CustomEvent('phonesin:open-profile-tab', { detail: 'notifications' }));
                                                    }, 0);
                                                }}
                                                className="text-[12px] font-black text-[#008d71] hover:underline"
                                            >
