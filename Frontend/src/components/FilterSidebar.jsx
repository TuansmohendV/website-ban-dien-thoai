...
const FilterSidebar = ({ category, filters, setFilters, onToggleFilter, onSetSingleFilter }) => {
  ...
  const setToggleFilter = (key, value) => {
    if (onToggleFilter) {
  ...
  const sectionFilterKey = (section) => {
    const keyMap = {
      storage: 'rom',
      battery: 'batteryTier',
      features: 'specialFeatures',
      screenTech: 'screenStandard',
    };
        ...
        <label key={opt} className="flex items-center justify-between cursor-pointer group">
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={() => setToggleFilter(key, opt)}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(() => {
                    ...
                    return displayedBrands.map((m) => (
                      <div
                        key={m.name}
                        onClick={() => setToggleFilter('brand', m.name)}
        ...
        {currentConfig.sections && currentConfig.sections.map(section => {
          ...
          return (
            <div key={section}>
              ...
              {expandedSections[section] && renderFilterList(sectionFilterKey(section), sectionOptions[section])}
            </div>
          );
        })}
